import java.util.*;

public class BreadtFirstSearch {


    public void traverseGraph(int[][] points, int startPoint) {

        // initial validation TODO

        // visited point track
        boolean[] visited = new boolean[points.length];
        // hold candidates
        Queue<Integer> candidatePoints = new LinkedList<>();
        candidatePoints.add(startPoint);
        visited[startPoint] = true;// mark start point visited

        int currentPoint = 0;
        int[] adjacentPoints = null;
        int nextAdjacent = 0;

        while (!candidatePoints.isEmpty()) {

            currentPoint = candidatePoints.poll();
            System.out.println("Traversing>" + currentPoint);

            adjacentPoints = points[currentPoint];

            for (int i = 0; i < adjacentPoints.length; i++) {

                nextAdjacent = adjacentPoints[i];
                boolean nextAdjacentVisisted = visited[nextAdjacent];
                if (!nextAdjacentVisisted) {

                    visited[nextAdjacent] = true;

                    candidatePoints.add(nextAdjacent);

                }

            }
        }


    }

    public void findShortestDistances(int[][] points, int startPoint, int endPoint) {

        // initial validation TODO

        // visited point track
        boolean[] visited = new boolean[points.length];
        // hold candidates
        PriorityQueue<Integer> candidatePoints = new PriorityQueue<>();
        candidatePoints.add(startPoint);
        visited[startPoint] = true;// mark start point visited
        int[] distances = new int[points.length];
        for (int i = 0; i < distances.length; i++) {
            distances[i] = Integer.MAX_VALUE;
        }
        distances[startPoint] = 0;
        Object[] previous = new Object[points.length];
        previous[startPoint] = null;
        int currentPoint = 0;
        int[] adjacentPoints = null;
        int nextAdjacent = 0;

        int distanceSoFar = 0;
        int popCount = 0;
        while (!candidatePoints.isEmpty()) {

            currentPoint = candidatePoints.poll();
            popCount++;
            System.out.println("Traversing>" + currentPoint);

            distanceSoFar = distances[currentPoint];
            adjacentPoints = points[currentPoint];

            for (int i = 0; i < adjacentPoints.length; i++) {

                nextAdjacent = adjacentPoints[i];

                int newDist = distanceSoFar + 1;

                if (newDist < distances[nextAdjacent]) {
                    distances[nextAdjacent] = newDist;
                    previous[nextAdjacent] = currentPoint;
                }


                boolean nextAdjacentVisisted = visited[nextAdjacent];
                if (!nextAdjacentVisisted) {

                    visited[nextAdjacent] = true;

                    candidatePoints.add(nextAdjacent);

                }

            }
        }

        System.out.println("--->PopCount::" + popCount);

        int idx = endPoint;
        List path = new ArrayList();
        path.add(endPoint);
        for (int i = endPoint; i != startPoint && previous[i] != null; i = (int) previous[i]) {

            path.add(previous[i]);

        }

        Collections.reverse(path);
        System.out.println("==>Path :" + path);


        Arrays.stream(distances).forEach(a -> System.out.println("mindistanceFromSource->" + a));

        System.out.println("---- result---");
        System.out.println(distances[endPoint]);
        System.out.println("stop");


    }

    public int findShortestDistances(int[][][] graph, int startPoint, int endPoint) {
        long startTime = System.currentTimeMillis();

        boolean isValidGraph = graph == null ? false : graph.length == 0 ? false : true;
        if (!isValidGraph)
            return -1;

        boolean istargetoverlaps = startPoint == endPoint;
        if (istargetoverlaps)
            return 0;

        int[] distances = new int[graph.length];

        for (int i = 0; i < distances.length; i++) {
            distances[i] = Integer.MAX_VALUE;
        }


        boolean[] visited = new boolean[graph.length];
        Queue<Integer> queue = new LinkedList<>();
        distances[startPoint] = 0;
        queue.add(distances[startPoint]);
        Object[] previous = new Object[graph.length];
        previous[startPoint] = null;
      //  visited[startPoint] = true;


        distances[startPoint] = 0;

        int currentPoint = 0;
        int[][] adjacentPointFeatures = null;
        int nextAdjacent = 0;
        int distanceSoFar = 0;

        int popCount = 0;
        while (!queue.isEmpty()) {

            currentPoint = queue.poll();
            popCount++;
             // System.out.println("Traversing>"+currentPoint);

            distanceSoFar = distances[currentPoint];
            adjacentPointFeatures = graph[currentPoint];

            for (int i = 0; i < adjacentPointFeatures.length; i++) {

                nextAdjacent = adjacentPointFeatures[i][0];
                // int weight = adjacentPointFeatures[i][1];
                int weight = 1;
                int newDist = distanceSoFar + weight;

                if (newDist < distances[nextAdjacent]) {
                    distances[nextAdjacent] = newDist;
                    previous[nextAdjacent] = currentPoint;

                    if (!visited[nextAdjacent]) {

                        visited[nextAdjacent] = true;
                        queue.add(nextAdjacent);
                    }
                }


            }
        }
        System.out.println("--->PopCount::" + popCount);

        int i = endPoint;
        List path = new ArrayList();
        while (previous[i] != null) {


            path.add(previous[i]);


            i = (int) previous[i];


        }
//        path.add(endPoint);
//        for (int i = endPoint; i != startPoint && previous[i] != null; i = (int) previous[i]) {
//
//            path.add(previous[i]);
//
//        }

        Collections.reverse(path);
        System.out.println("==>Path :" + path);


//        for(int i = 0; i < distances.length; i++){
//
//            System.out.println(startPoint+"->"+i+":"+distances[i]);
//
//        }

        System.out.println("---- result---");
        System.out.println(distances[endPoint]);
        System.out.println("stop");

        long endTime = System.currentTimeMillis();
        System.out.println("Total execution time: " + (endTime - startTime) + "ms");

        return distances[endPoint];
    }

    public static void main(String[] args) {

//        int [][] input = {
//                {1},
//                {0,3},
//                {3,4,5},
//                {1,2,4},
//                {2,3},
//                {2}
//        };

        int[][] input = {
                {5, 3, 6, 1},
                {6, 0, 3, 4, 2},
                {4, 1},
                {5, 0, 1, 4},
                {2, 1, 3},
                {0, 3},
                {0, 1}
        };

        int[][][] info = {
                {{1, 6}, {3, 1}, {5, 1}, {6, 5}},
                {{0, 6}, {3, 2}, {4, 2}, {2, 5}, {6, 2}},
                {{1, 5}, {4, 5}},
                {{0, 1}, {1, 2}, {4, 1}, {5, 7}},
                {{1, 2}, {2, 5}, {3, 1}},
                {{0, 1}, {3, 7}},
                {{1, 5}, {1, 2}}
        };


        BreadtFirstSearch breadtFirstSearch = new BreadtFirstSearch();
        breadtFirstSearch.findShortestDistances(SampleInputs.get18weightedShortestPath(), 0, 9);

    }
}
